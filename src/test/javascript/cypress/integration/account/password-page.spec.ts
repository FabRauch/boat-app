...
describe('/account/password', () => {
  ...
  it('requires new password', () => {
    cy.get(submitPasswordSelector).click();
    cy.get(newPasswordSelector).should('have.class', classInvalid).type('boat').blur().should('have.class', classValid);
  });

  it('requires confirm new password', () => {
    cy.get(submitPasswordSelector).click();
    cy.get(newPasswordSelector).type('boat');
    cy.get(confirmPasswordSelector).should('have.class', classInvalid).type('boat').blur().should('have.class', classValid);
  });
  ...
  it('should fail to update password when using incorrect current password', () => {
    ...
    cy.get(newPasswordSelector).type('boat');
    cy.get(confirmPasswordSelector).type('boat');
    cy.get(submitPasswordSelector).click();
